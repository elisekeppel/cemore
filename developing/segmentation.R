...
#=========================================== CEMORE: NO STRATIFICATION REQUIRED ============================================#
...
# In this step, we want to create lines that are continuous except in OFF-effort portions AND when the transect
...
#--------------------------------------------
# EK: This could probably be done simply by creating lines for each ONSEQ_ID as they
# are already broken up by changes in Effort Status. But this code works so why
# reinvent the wheel ;)
#--------------------------------------------
...
# EK edit to use purrr package
x <- split(Epoints, Epoints$line.code) %>% 
  purrr::map( ~ Lines(list(Line(coordinates(.))), ID = .$line.code[1]))

# Convert the list of lines to a SpatialLines object (unprojected)
l <- SpatialLines(x, proj4string=CRS("+proj=longlat"))
# EK edit: eg. line length
View(l[1]@lines$`1`@Lines[[1]]@coords)
plot(l[1]@lines$`1`@Lines[[1]]@coords)
LineLength(l[1]@lines$`1`@Lines[[1]]@coords, longlat = TRUE) * 1000
...
range(l.length) #[1] 158.6748 164553.7803 m # EK for CeMoRe July 2021 survey,  1298.905 m to 19023.740 m
...
writeOGR(sldf, dsn=getwd(), layer=paste0("cemore_dataEffort_", year, "_", month,  "_LinesCoded_Unsegmented"), driver="ESRI Shapefile", overwrite_layer=TRUE)
...
# setwd("C:/Users/WrightBri/Documents/PRISMM/ANALYSIS")
EffortLines<-readOGR(dsn=getwd(), layer=paste0("cemore_dataEffort_", year, "_", month,  "_LinesCoded_Unsegmented"))
# Get data from lines
